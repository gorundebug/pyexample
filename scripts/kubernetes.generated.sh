...
NAMESPACE="${KUBERNETES_NAMESPACE:-pyexample}"
...
deploy_services() {
  ...
  helm upgrade --install analyticsservice \
    ...
    --set-string image.repository="${CLUSTER_REGISTRY}/pyexample/analyticsservice" \
  ...
  helm upgrade --install automationservice \
    ...
    --set-string image.repository="${CLUSTER_REGISTRY}/pyexample/automationservice" \
    --set-string image.tag="${IMAGE_TAG}" \
    "$@"
  set --
  if [[ "${KAFKA_SASL_ENABLED}" == "true" ]]; then
  ...
  helm upgrade --install inventoryservice \
    ...
    --set-string image.repository="${CLUSTER_REGISTRY}/pyexample/inventoryservice" \
    --set-string image.tag="${IMAGE_TAG}" \
    "$@"
  set --
  if [[ "${KAFKA_SASL_ENABLED}" == "true" ]]; then
  ...
  helm upgrade --install orderservice \
    ...
    --set-string image.repository="${CLUSTER_REGISTRY}/pyexample/orderservice" \
    --set-string image.tag="${IMAGE_TAG}" \
    "$@"
  set --
  if [[ "${KAFKA_SASL_ENABLED}" == "true" ]]; then
