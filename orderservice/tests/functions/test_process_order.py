"""Behavior tests for the user-owned ProcessOrder handler."""
...
from order_service.internal.functions.process_order import ProcessOrder
...
@pytest.mark.asyncio
async def test_process_order_aggregates_results_into_openapi_response() -> None:
...
@pytest.mark.asyncio
async def test_process_order_rejects_empty_items() -> None:
    function = ProcessOrder()
    stream = StreamContext()
    data = Data({"items": []})
    result = ResultContext()
