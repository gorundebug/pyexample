...
_DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "streams": { "mapOrderItemResultToOrderState": StreamConfig(functionDescription="Produce an order result containing one inventory result and preserving its order ID.\nMark it CONFIRMED when the item was reserved; otherwise mark it PARTIALLY_CONFIRMED.\nRecord the time when this result is produced.\n", functionInitializerGroup="", functionModule="", functionName="MapOrderItemResultToOrderState", functionPackage="", id=26, idService=4, idSource=31, name="Map Order Item Result To Order State", pipeline="order", type=TransformationType(2), valueType="OrderState", xPos=103, yPos=-52, ), "mapToOrderProcessed": StreamConfig(functionDescription="Create an OrderProcessed event from the final order state.\nPreserve the order ID, status, and processing time. Count all item results and reserved items; for unsuccessful orders use the final status as the failure reason.\n", functionInitializerGroup="", functionModule="", functionName="MapToOrderProcessed", functionPackage="", id=27, idService=4, idSource=35, name="MapToOrderProcessed", pipeline="order", type=TransformationType(2), valueType="OrderProcessed", xPos=-821, yPos=-26, ), "mapToOrderState": StreamConfig(functionDescription="Produce a TIMED_OUT order result that preserves the order ID and submitted total.\nDo not add item results at this stage; results received before the timeout are included in the final response.\n", functionInitializerGroup="", functionModule="", functionName="MapToOrderState", functionPackage="", id=28, idService=4, idSource=34, name="Map to Order State", pipeline="order", type=TransformationType(2), valueType="OrderState", xPos=-368, yPos=-227, ), "mergeResults": StreamConfig(id=29, idService=4, idSource=0, idSources=[28, 26, 25], name="Merge Results", pipeline="order", type=TransformationType(10), xPos=-228, yPos=130, ), "processOrder": StreamConfig(id=30, idEndpoint=4, idService=4, idSource=35, name="Process Order", pipeline="order", type=TransformationType(1), valueType="Order", xPos=-760, yPos=-367, ), "processOrderItem": StreamConfig(id=31, idEndpoint=1, idService=4, idSource=32, name="Process Order Item", pipeline="order", type=TransformationType(13), valueType="OrderItemResult", xPos=-60, yPos=-375, ), "processOrderItems": StreamConfig(functionDescription="Emit every order item independently for inventory processing.\nPreserve each item's data and assign the parent order ID.\n", functionInitializerGroup="", functionModule="", functionName="ProcessOrderItems", functionPackage="", id=32, idService=4, idSource=36, name="Process Order Items", pipeline="order", type=TransformationType(7), valueType="OrderItem", xPos=-198, yPos=-662, ), "publishOrderProcessed": StreamConfig(id=33, idEndpoint=3, idService=4, idSource=27, name="Publish Order Processed", pipeline="order", type=TransformationType(13), valueType="OrderProcessed", xPos=-944, yPos=-255, ), "softDeadline": StreamConfig(duration=1000, functionDescription="Trigger the timeout branch shortly before the request deadline, leaving the configured duration to assemble a response.\nWhen no request deadline exists, use the configured duration itself. Never wait past an existing deadline.\n", functionInitializerGroup="", functionModule="", functionName="SoftDeadline", functionPackage="", id=34, idService=4, idSource=36, name="Soft Deadline", pipeline="order", type=TransformationType(16), xPos=-477, yPos=-444, ), "splitOrderResult": StreamConfig(id=35, idService=4, idSource=29, name="Split Order Result", pipeline="order", type=TransformationType(11), xPos=-671, yPos=-129, ), "splitPipeline": StreamConfig(id=36, idService=4, idSource=30, name="Split Pipeline", pipeline="order", type=TransformationType(11), xPos=-640, yPos=-597, ), },
    ...
    "links": { "mergeResultsToSplitOrderResult": LinkConfig(callSemantics=CallSemantics(2), var_from=29, to=35, ), "processOrderToSplitPipeline": LinkConfig(callSemantics=CallSemantics(2), var_from=30, poolName="Default Pool", priority=1, to=36, ), "splitPipelineToProcessOrderItems": LinkConfig(callSemantics=CallSemantics(2), var_from=36, to=32, ), "splitPipelineToSoftDeadline": LinkConfig(var_async=True, callSemantics=CallSemantics(2), var_from=36, to=34, ), },
...
class StreamIds:
    MAP_ORDER_ITEM_RESULT_TO_ORDER_STATE: Final[int] = 26
    MAP_TO_ORDER_PROCESSED: Final[int] = 27
    MAP_TO_ORDER_STATE: Final[int] = 28
    MERGE_RESULTS: Final[int] = 29
    PROCESS_ORDER: Final[int] = 30
    PROCESS_ORDER_ITEM: Final[int] = 31
    PROCESS_ORDER_ITEMS: Final[int] = 32
    PUBLISH_ORDER_PROCESSED: Final[int] = 33
    SOFT_DEADLINE: Final[int] = 34
    SPLIT_ORDER_RESULT: Final[int] = 35
    SPLIT_PIPELINE: Final[int] = 36
