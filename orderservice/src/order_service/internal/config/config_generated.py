...
_DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "streams": { "mapOrderItemResultToOrderState": StreamConfig(functionDescription="Produce an order result containing one inventory result and preserving its order ID.\nMark it CONFIRMED when the item was reserved; otherwise mark it PARTIALLY_CONFIRMED.\nRecord the time when this result is produced.\n", functionInitializerGroup="", functionModule="", functionName="MapOrderItemResultToOrderState", functionPackage="order", id=65, idService=4, idSource=70, name="Map Order Item Result To Order State", pipeline="order", type=TransformationType(2), valueType="OrderState", xPos=103, yPos=-52, ), "mapToOrderProcessed": StreamConfig(functionDescription="Create an OrderProcessed event from the final order state.\nPreserve the order ID, status, and processing time. Count all item results and reserved items; for unsuccessful orders use the final status as the failure reason.\n", functionInitializerGroup="", functionModule="", functionName="MapToOrderProcessed", functionPackage="order", id=66, idService=4, idSource=74, name="MapToOrderProcessed", pipeline="order", type=TransformationType(2), valueType="OrderProcessed", xPos=-821, yPos=-26, ), "mapToOrderState": StreamConfig(functionDescription="Produce a TIMED_OUT order result that preserves the order ID and submitted total.\nDo not add item results at this stage; results received before the timeout are included in the final response.\n", functionInitializerGroup="", functionModule="", functionName="MapToOrderState", functionPackage="order", id=67, idService=4, idSource=73, name="Map to Order State", pipeline="order", type=TransformationType(2), valueType="OrderState", xPos=-368, yPos=-227, ), "mergeResults": StreamConfig(id=68, idService=4, idSource=0, idSources=[67, 65, 64], name="Merge Results", pipeline="order", type=TransformationType(10), xPos=-228, yPos=130, ), "processOrder": StreamConfig(id=69, idEndpoint=11, idService=4, idSource=74, name="Process Order", pipeline="order", type=TransformationType(1), valueType="Order", xPos=-760, yPos=-367, ), "processOrderItem": StreamConfig(id=70, idEndpoint=7, idService=4, idSource=71, name="Process Order Item", pipeline="order", type=TransformationType(13), valueType="OrderItemResult", xPos=-60, yPos=-375, ), "processOrderItems": StreamConfig(functionDescription="Emit every order item independently for inventory processing.\nPreserve each item's data and assign the parent order ID.\n", functionInitializerGroup="", functionModule="", functionName="ProcessOrderItems", functionPackage="order", id=71, idService=4, idSource=75, name="Process Order Items", pipeline="order", type=TransformationType(7), valueType="OrderItem", xPos=-198, yPos=-662, ), "publishOrderProcessed": StreamConfig(id=72, idEndpoint=10, idService=4, idSource=66, name="Publish Order Processed", pipeline="order", type=TransformationType(13), valueType="OrderProcessed", xPos=-944, yPos=-255, ), "softDeadline": StreamConfig(duration=1000, functionDescription="Trigger the timeout branch shortly before the request deadline, leaving the configured duration to assemble a response.\nWhen no request deadline exists, use the configured duration itself. Never wait past an existing deadline.\n", functionInitializerGroup="", functionModule="", functionName="SoftDeadline", functionPackage="order", id=73, idService=4, idSource=75, name="Soft Deadline", pipeline="order", type=TransformationType(16), xPos=-477, yPos=-444, ), "splitOrderResult": StreamConfig(id=74, idService=4, idSource=68, name="Split Order Result", pipeline="order", type=TransformationType(11), xPos=-671, yPos=-129, ), "splitPipeline": StreamConfig(id=75, idService=4, idSource=69, name="Split Pipeline", pipeline="order", type=TransformationType(11), xPos=-640, yPos=-597, ), },
    "dataConnectors": { "inventoryServiceApi": DataConnectorConfig(address="dns:///localhost:9202", connectionsCount=1, id=2, implementation="google/grpc", module="inventory_service_api", name="Inventory Service API", type=DataConnectorType(2), ), "orderEvents": DataConnectorConfig(brokers="redpanda:9092", dialTimeout=5000, id=4, implementation="aiokafka", name="Order Events", password="", saslMechanism=KafkaSaslMechanism("SCRAM-SHA-512"), securityProtocol=KafkaSecurityProtocol("PLAINTEXT"), type=DataConnectorType(3), username="", version="2.8.0", ), "orderServiceApi": DataConnectorConfig(id=5, implementation="aiohttp", module="order_service_api", name="Order Service API", type=DataConnectorType(1), useDedicatedListener=False, ), },
    "endpoints": { "orderProcessed": EndpointConfig(consumerGroup="analytics-service", createTopic=True, enabled=True, functionDescription="Exchange OrderProcessed events keyed by order ID.\nProducers include the final status, processing time, total and confirmed item counts, and a failure reason for unsuccessful orders.\nConsumers decode the event and mark its Kafka message processed only after the pipeline handles it successfully.\n", functionInitializerGroup="", functionName="OrderProcessedEndpoint", functionPackage="endpoint", id=10, idDataConnector=4, name="Order Processed", partitions=1, publicFunction=False, replicationFactor=1, topic="order-processed", ), "processOrder": EndpointConfig(functionDescription="Accept orders with at least one item and positive quantities; reject malformed or invalid requests as client errors.\nReuse X-Request-ID when supplied, otherwise generate an order ID. Preserve customer, item, price, and X-Trace data, and apply the configured timeout of five seconds by default.\nReturn one response per order. When all items finish, use CONFIRMED only if every item was reserved; otherwise use PARTIALLY_CONFIRMED. If the deadline wins, return TIMED_OUT with the item results received so far.\nCalculate the total from processed item prices, falling back to the submitted total when no item result arrived, and include individual item failures in the response.\n", functionInitializerGroup="", functionName="ProcessOrder", functionPackage="endpoint", httpMethodType=HTTPMethodType("POST"), id=11, idDataConnector=5, name="Process Order", path="/v1/processorder", publicFunction=False, ), "processOrderItem": EndpointConfig(functionDescription="Reserve inventory for one order item using its order ID, item ID, SKU, and quantity.\nReturn the available quantity, reservation outcome, and status. The caller combines this response with the original identity, requested quantity, and unit price.\nIf the inventory call fails, the caller returns a non-reserved PROCESSING_ERROR result with the failure message.\n", functionInitializerGroup="", functionName="ProcessOrderItem", functionPackage="endpoint", grpcMethodType=GrpcMethodType(1), id=7, idDataConnector=2, methodName="ProcessOrderItem", name="Process Order Item", publicFunction=False, ), },
    "pools": { "defaultPool": PoolConfig(executorsCount=2, name="Default Pool", ), },
    "links": { "mergeResultsToSplitOrderResult": LinkConfig(callSemantics=CallSemantics(2), var_from=68, to=74, ), "processOrderToSplitPipeline": LinkConfig(callSemantics=CallSemantics(2), var_from=69, poolName="Default Pool", priority=1, to=75, ), "splitPipelineToProcessOrderItems": LinkConfig(callSemantics=CallSemantics(2), var_from=75, to=71, ), "splitPipelineToSoftDeadline": LinkConfig(var_async=True, callSemantics=CallSemantics(2), var_from=75, to=73, ), },
...
class StreamIds:
    MAP_ORDER_ITEM_RESULT_TO_ORDER_STATE: Final[int] = 65
    MAP_TO_ORDER_PROCESSED: Final[int] = 66
    MAP_TO_ORDER_STATE: Final[int] = 67
    MERGE_RESULTS: Final[int] = 68
    PROCESS_ORDER: Final[int] = 69
    PROCESS_ORDER_ITEM: Final[int] = 70
    PROCESS_ORDER_ITEMS: Final[int] = 71
    PUBLISH_ORDER_PROCESSED: Final[int] = 72
    SOFT_DEADLINE: Final[int] = 73
    SPLIT_ORDER_RESULT: Final[int] = 74
    SPLIT_PIPELINE: Final[int] = 75


class EndpointIds:
    ORDER_PROCESSED: Final[int] = 10
    PROCESS_ORDER: Final[int] = 11
    PROCESS_ORDER_ITEM: Final[int] = 7
