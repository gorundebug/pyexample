...
[Private helper] converts OrderState to ProcessOrderResponse mapping all fields including optional ConfirmedItems slice."""
    ...
    async def consume_message(
        ...
        result_ctx: ResultContext[
            ProcessOrderState, Order, OrderState, Exception
        ],
    ) -> None:
        ...
        for raw_item in raw_items:
            ...
            if quantity <= 0:
                raise web.HTTPBadRequest(
                    text="all quantities must be positive"
                )
            unit_price = float(
                _field(raw_item, "unitPrice", "unit_price", default=0.0)
            )
            item = OrderItem(
                order_id=order_id,
        ...
        order = Order(
            id=order_id,
            customer_id=str(
                _field(body, "customerId", "customer_id", default="")
            ),
            items=items,
        ...
        def on_result(
            ...
            if value.status != "TIMED_OUT":
                ...
                if (
                    len(handler_state.results)
                    < handler_state.expected_items
                ):
                    return False
            ...
            total = sum(
                result.unit_price * result.requested_qty
                for result in handler_state.results
            )
