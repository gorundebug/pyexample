...
_DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "streams": { "getInventoryItemData": StreamConfig(functionDescription="Reserve the requested quantity without allowing concurrent orders to overdraw stock.\nOn success, return CONFIRMED with the requested quantity available. Otherwise return OUT_OF_STOCK with the current available quantity.\nPreserve the order and item identity, requested quantity, and unit price.\nThe example starts with SKU-001: 100, SKU-002: 50, and SKU-003: 25.\n", functionInitializerGroup="", functionModule="", functionName="GetInventoryItemData", functionPackage="inventoryItem", id=73, idService=3, idSource=77, name="Get Inventory Item Data", pipeline="inventoryItem", type=TransformationType(6), valueType="OrderItemResult", xPos=527, yPos=-562, ), "mapInventoryItemError": StreamConfig(functionDescription="When inventory processing fails, return an OUT_OF_STOCK result with no available quantity.\nPreserve the order and item identity and requested quantity, and record the failure.\n", functionInitializerGroup="", functionModule="", functionName="GetInventoryItemError", functionPackage="inventoryItem", id=75, idService=3, idSource=-73, name="Map Inventory Item Error", pipeline="inventoryItem", type=TransformationType(2), valueType="OrderItemResult", xPos=733, yPos=-263, ), "mergeInventoryResult": StreamConfig(id=76, idService=3, idSource=0, idSources=[73, 75], name="Merge Inventory Result", pipeline="inventoryItem", type=TransformationType(10), xPos=542, yPos=33, ), "processInventoryItem": StreamConfig(id=77, idEndpoint=11, idService=3, idSource=76, name="Process Inventory Item", pipeline="inventoryItem", type=TransformationType(1), valueType="OrderItem", xPos=250, yPos=-400, ), },
    "dataConnectors": { "inventoryServiceApi": DataConnectorConfig(address="dns:///localhost:9202", connectionsCount=1, id=2, implementation=DataConnectorImplementation("google/grpc"), module="inventory_service_api", name="Inventory Service API", type=DataConnectorType(2), ), },
    ...
    "links": { "getInventoryItemDataToMergeInventoryResult": LinkConfig(callSemantics=CallSemantics(2), var_from=73, to=76, ), "processInventoryItemToGetInventoryItemData": LinkConfig(callSemantics=CallSemantics(2), var_from=77, poolName="Inventory Priority Workers", priority=10, to=73, ), },
    ...
    "types": { "inventoryFailure": TypeConfig(definitionFormat=TypeDefinitionFormat(1), description="Inventory shortage data passed to GetInventoryItemError. Preserves the original order item and available quantity without throwing an exception.", name="InventoryFailure", publicType=False, type=DataType.struct, ), "orderItem": TypeConfig(definitionFormat=TypeDefinitionFormat(1), description="A single line item within an order. Fields: OrderID string, ItemID string, SKU string, Quantity int.", module="model", name="OrderItem", publicType=False, transferByValue=False, type=DataType.struct, ), "orderItemResult": TypeConfig(definitionFormat=TypeDefinitionFormat(1), description="Inventory reservation result for a single order item. Fields: OrderID string, ItemID string, SKU string, RequestedQty int, AvailableQty int, Reserved bool, Status string (CONFIRMED / OUT_OF_STOCK / PROCESSING_ERROR), UnitPrice float64, Error string.", module="model", name="OrderItemResult", publicType=False, transferByValue=False, type=DataType.struct, ), },
}
...
class StreamIds:
    GET_INVENTORY_ITEM_DATA: Final[int] = 73
    MAP_INVENTORY_ITEM_ERROR: Final[int] = 75
    MERGE_INVENTORY_RESULT: Final[int] = 76
    PROCESS_INVENTORY_ITEM: Final[int] = 77
...
class Streams:
    ...
    map_inventory_item_error: MapStreamConfig
    ...
    def named(self) -> NamedConfig:
        return NamedConfig(
            streams=Streams(
                ...
                map_inventory_item_error=MapStreamConfig(
                    self.get_stream_config_by_id(StreamIds.MAP_INVENTORY_ITEM_ERROR)
                ),
                merge_inventory_result=MergeStreamConfig(
