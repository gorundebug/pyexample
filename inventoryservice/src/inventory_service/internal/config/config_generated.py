...
_DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "streams": { "getInventoryItemData": StreamConfig(functionDescription="Reserve the requested quantity without allowing concurrent orders to overdraw stock.\nOn success, return CONFIRMED with the requested quantity available. Otherwise return OUT_OF_STOCK with the current available quantity.\nPreserve the order and item identity, requested quantity, and unit price.\nThe example starts with SKU-001: 100, SKU-002: 50, and SKU-003: 25.\n", functionInitializerGroup="", functionModule="", functionName="GetInventoryItemData", functionPackage="inventoryItem", id=73, idService=3, idSource=76, name="Get Inventory Item Data", pipeline="inventoryItem", type=TransformationType(6), valueType="OrderItemResult", xPos=527, yPos=-562, ), "mergeInventoryResult": StreamConfig(id=75, idService=3, idSource=0, idSources=[73, 74], name="Merge Inventory Result", pipeline="inventoryItem", type=TransformationType(10), xPos=542, yPos=33, ), "processInventoryItem": StreamConfig(id=76, idEndpoint=11, idService=3, idSource=75, name="Process Inventory Item", pipeline="inventoryItem", type=TransformationType(1), valueType="OrderItem", xPos=250, yPos=-400, ), },
    ...
    "links": { "getInventoryItemDataToMergeInventoryResult": LinkConfig(callSemantics=CallSemantics(2), var_from=73, to=75, ), "processInventoryItemToGetInventoryItemData": LinkConfig(callSemantics=CallSemantics(2), var_from=76, poolName="Inventory Priority Workers", priority=10, to=73, ), },
...
class StreamIds:
    ...
    MERGE_INVENTORY_RESULT: Final[int] = 75
    PROCESS_INVENTORY_ITEM: Final[int] = 76
