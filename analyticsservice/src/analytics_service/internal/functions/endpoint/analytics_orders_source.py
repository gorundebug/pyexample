...
class AnalyticsOrdersSource:
    ...
    async def start(
        ...
        del ctx
        await consumer.consume(AnalyticsEvent(key="high-value", value=10, kind="order"))
        await consumer.consume(AnalyticsEvent(key="standard", value=1, kind="order"))
...
async def make_analytics_orders_source(
    ctx: Context,
    environment: ServiceEnvironment,
    config: CustomEndpointConfig,
) -> AnalyticsOrdersSource:
    """Construct AnalyticsOrdersSource asynchronously while the graph is initialized."""
    del ctx, config, environment
    return AnalyticsOrdersSource()
