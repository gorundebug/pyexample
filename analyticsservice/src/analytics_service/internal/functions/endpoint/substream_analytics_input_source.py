...
class SubstreamAnalyticsInputSource:
    ...
    async def start(
        ...
        del ctx
        await consumer.consume(AnalyticsEvent(key="substream", value=7, kind="input"))
    ...
    async def consume_message(
        ...
        result_ctx: ResultContext[
            None, AnalyticsEvent, AnalyticsEvent, Exception
        ],
    ) -> None:
...
async def make_substream_analytics_input_source(
    ctx: Context,
    environment: ServiceEnvironment,
    config: CustomEndpointConfig,
) -> SubstreamAnalyticsInputSource:
    """Construct SubstreamAnalyticsInputSource asynchronously while the graph is initialized."""
    del ctx, config, environment
    return SubstreamAnalyticsInputSource()
